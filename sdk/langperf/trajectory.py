...
def trajectory(
    ...
    *,
    ...
    Parameters:
        ...
        final: Whether this `with` block is the last segment of the run.
            When False, `__exit__` skips stamping `langperf.completed` on
            the root span so a durable run split across multiple segments
            only finalizes its completion status when the last segment
            closes. Defaults to True (the classic single-segment case).
